...
namespace EDSc.Common.Utils
{
    public class ImgToMongoWriter : IImgToDbWriter<ImageDto>, IDisposable
    {
        ...
        public string SaveToDb(ImageDto img)
        {
            var collection = this.Client.GetDatabase(this.DbName).GetCollection<MongoImage>(this.CollectionName);

            var mongoImg = new MongoImage
            {
                Id = img.Id,
                Url = img.Url,
                Score = img.Score,
